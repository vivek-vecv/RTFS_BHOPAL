...
import "./Gen.css"; // Import the custom CSS
...
const Genealogy = () => {
    ...
    const fetchGenealogyData = async () => {
      ...
      try {
        ...
        if (
          response.status === 200 &&
          response.data &&
          response.data.Geneaology_Information
        ) {
  ...
  const handleToggleChange = (index) => {
    setData((prevState) => {
      const updatedTableData = prevState.tableData.map((row, i) =>
        i === index ? { ...row, newTracibility: !row.newTracibility } : row
      );
  ...
  const handleInputChange = (index, value) => {
    setData((prevState) => {
      const updatedTableData = prevState.tableData.map((row, i) =>
        i === index ? { ...row, newTracibilityValue: value } : row
      );
      return { ...prevState, tableData: updatedTableData };
    });
  };

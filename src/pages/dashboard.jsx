...
import { CRow, CCol, CFormLabel, CFormSelect, CButton, CNavbar, CNavbarBrand, CNavbarText } from '@coreui/react';
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchRoutes = async () => {
      try {
        const response = await axios.get(`http://10.119.1.101:9898/rest/api/getAllRouteNames`, {
          auth: {
            username: 'Arun',
            password: '123456',
          },
        ...
        const routeNames = response.data.Route_Information.map((route) => route.Route_Name);
  ...
  const fetchSteps = async (routeNames) => {
    try {
      ...
      const response = await axios.get(
        `http://10.119.1.101:9898/rest/api/getStepsByRouteName?routename=${encodeURIComponent(routeNames)}`,
        {
          auth: {
            username: 'Arun',
            password: '123456',
      ...
      if (response.data.Step_Information) {
        const steps = response.data.Step_Information.map((step) => step.Step_Name);
        setRouteSteps(steps);
    ...
    <div className="full-screen">
      ...
      <CRow className="header mb-3">
        <CCol xs={12} md={4} className="header-item">
          ...
          <CFormSelect value={route} onChange={handleRouteChange} className="mb-3">
        ...
        <CCol xs={12} md={4} className="header-item">
          ...
          <CFormSelect value={routeStep} onChange={handleRouteStepChange} disabled={routeStepDisabled} className="mb-3">
        ...
        <CCol xs={12} md={4} className="header-item">
          ...
          <CFormSelect value={stationSide} onChange={handleStationSideChange} disabled={stationSideDisabled} className="mb-3">
